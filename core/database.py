...
# ---------------------------------------------------------------------------
# Google Drive Organizer — Batch 19 persistent planning models
# ---------------------------------------------------------------------------

class GoogleDriveOrganizerScanRun(TimestampMixin, Base):
    __tablename__ = "google_drive_organizer_scan_runs"
    ...
    file_count_scanned = Column(Integer, nullable=False, default=0)
    file_count_indexed = Column(Integer, nullable=False, default=0)
    ...
    __table_args__ = (
        Index("ix_gdrive_organizer_scan_owner_created", "owner", "created_at"),
    )


class GoogleDriveOrganizationPlan(TimestampMixin, Base):
    __tablename__ = "google_drive_organization_plans"
    ...
    scan_run_id = Column(
        String,
        ForeignKey(
            "google_drive_organizer_scan_runs.id",
            ondelete="CASCADE",
        ),
        nullable=False,
        index=True,
    ...
    __table_args__ = (
        Index("ix_gdrive_organizer_plan_owner_created", "owner", "created_at"),
    )


class GoogleDriveOrganizationAction(TimestampMixin, Base):
    __tablename__ = "google_drive_organization_actions"
    ...
    plan_id = Column(
        String,
        ForeignKey(
            "google_drive_organization_plans.id",
            ondelete="CASCADE",
        ),
        nullable=False,
        index=True,
    ...
    remote_file_id = Column(String, nullable=True, index=True)
    ...
    proposed = Column(JSON, nullable=False, default=dict)

    status = Column(String, nullable=False, default="proposed", index=True)
    requires_approval = Column(Boolean, nullable=False, default=True)
    approval_fingerprint = Column(String, nullable=True, index=True)
    execution_result = Column(JSON, nullable=True)

    __table_args__ = (
        Index("ix_gdrive_organizer_action_owner_plan", "owner", "plan_id"),
    )


class GoogleDriveOrganizationApproval(TimestampMixin, Base):
    __tablename__ = "google_drive_organization_approvals"
    ...
    tool_name = Column(
        String,
        nullable=False,
        default="google_drive_apply_plan",
    )
    selection_mode = Column(String, nullable=False, default="all")
    action_ids = Column(JSON, nullable=False, default=list)

    invocation_fingerprint = Column(String, nullable=False, index=True)
    request_preview = Column(JSON, nullable=False, default=dict)
    ...
    __table_args__ = (
        Index("ix_gdrive_organizer_approval_owner_status", "owner", "status"),
    )
