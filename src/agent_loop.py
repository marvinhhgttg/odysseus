...
- "Morgenbriefing" / "morning briefing" / "Tagesbriefing" / daily morning overview of mail+calendar+tasks → call `get_briefing` (run synchronously, show the markdown in chat). Do NOT hand-assemble the daily overview from separate list_emails/manage_calendar/manage_notes calls — get_briefing does the per-mail analysis, conflict detection and Google Tasks overview in one shot."
