...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "get_briefing": "Generate the morning briefing ('Morgenbriefing') for the user. Assembles the rich German daily overview: per-mail LLM summaries with importance/urgency, today's calendar + 7-day outlook with source tags and conflict fixes, open Google Tasks buckets, top-5 actions, and display-only proposals ([K] calendar deep links, [D] drive filing, [E] reply drafts). Use when the user asks for 'Morgenbriefing', 'morning briefing', 'Tagesbriefing' or a daily overview of mail/calendar/tasks. Run synchronously and show the result in chat.",
