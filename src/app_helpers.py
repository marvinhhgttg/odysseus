...
def setup_environment() -> None:
    """Configure global environment hacks for cross-platform consistency.
    
    Handles Windows-specific loop policies, MIME type registration for ES modules,
    and HuggingFace symlink overrides to ensure stable startup across different OSs.
    """
    # On Windows, asyncio.create_subprocess_exec/shell require the ProactorEventLoop.
    if sys.platform == "win32":
        asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())

    # Force stable JS module MIME types across platforms to prevent browser errors.
...
def serve_html_with_nonce(request: Request, file_path: str) -> HTMLResponse:
    ...
    template surfaces inT 5xx alerting instead of hiding behind a 404. If a
